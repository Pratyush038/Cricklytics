    ...
    if model_category == "Batsman":
        ...
        with col1:
            with st.container():
                ...
                matches = st.number_input("Matches Played", min_value=1)
        ...
        with col2:
            with st.container():
                st.markdown("<div class='card'>", unsafe_allow_html=True)
                st.markdown("<div class='card-content'>", unsafe_allow_html=True)
                average = st.number_input("Batting Average", min_value=0.0, format="%.2f")
                boundary_percent = st.slider("Boundary Percentage", 0.0, 100.0, format="%.1f")
                start_year = st.number_input("Career Start Year", min_value=1950, max_value=datetime.now().year)
                end_year = st.number_input("Career End Year (Current year if active)", min_value=1950, max_value=datetime.now().year)
                st.markdown("</div></div>", unsafe_allow_html=True)
        ...
        with col2:
            with st.container():
                st.markdown("<div class='card'>", unsafe_allow_html=True)
                st.markdown("<div class='card-content'>", unsafe_allow_html=True)
                economy = st.number_input("Economy Rate", min_value=0.0, format="%.2f")
                strike_rate = st.number_input("Bowling Strike Rate", min_value=0.0, format="%.2f")
                start_year = st.number_input("Career Start Year", min_value=1950, max_value=datetime.now().year)
                end_year = st.number_input("Career End Year (Current year if active)", min_value=1950, max_value=datetime.now().year)
                st.markdown("</div></div>", unsafe_allow_html=True)
